...
export default function BoldTemplate({ resume, dark: D }: { resume: ResumeOutput; dark: boolean }) {
  ...
  return (
    <div id="resume-output" style={{
      ...
      fontFamily: 'Inter, sans-serif', fontSize: 13, lineHeight: 1.6, color: textC,
      boxShadow: D ? '0 1px 3px rgba(0,0,0,0.4)' : '0 2px 16px rgba(0,0,0,0.08)',
      overflow: 'hidden',
    }}>
      {/* Bold full-width header */}
      <div style={{ background: D ? '#111827' : '#0f172a', padding: '36px 40px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: 16 }}>
          <div>
            <h1 style={{ fontSize: 32, fontWeight: 900, color: '#fff', margin: '0 0 4px', letterSpacing: '-0.03em', lineHeight: 1.1 }}>{resume.name}</h1>
            {resume.title && (
              <div style={{ display: 'inline-block', background: '#e11d48', padding: '3px 12px', borderRadius: 4, marginTop: 6 }}>
                <p style={{ fontSize: 12, fontWeight: 700, color: '#fff', margin: 0, textTransform: 'uppercase', letterSpacing: '0.06em' }}>{resume.title}</p>
              </div>
            )}
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4, fontSize: 11, color: 'rgba(255,255,255,0.6)', textAlign: 'right' }}>
      ...
      <div style={{ padding: '32px 40px' }}>
        {resume.summary && (
          <div style={{ marginBottom: 24 }}>
            ...
            <p style={{ fontSize: 12.5, lineHeight: 1.85, margin: '10px 0 0', color: textMuted }}>{resume.summary}</p>
          </div>
        )}
              ...
              {resume.experience.map((exp, i) => (
                <div key={i} style={{ marginBottom: i < resume.experience.length - 1 ? 18 : 0, paddingBottom: i < resume.experience.length - 1 ? 18 : 0, borderBottom: i < resume.experience.length - 1 ? `1px dashed ${borderCol}` : 'none' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', marginBottom: 4 }}>
                    <span style={{ fontWeight: 800, fontSize: 14, color: textC }}>{exp.role}</span>
                    <span style={{ fontSize: 11, fontWeight: 600, color: '#e11d48', background: D ? '#2d1520' : '#fff1f2', padding: '2px 10px', borderRadius: 4 }}>{exp.period}</span>
                  </div>
                  <p style={{ fontSize: 12, color: textMuted, fontWeight: 600, margin: '0 0 6px' }}>{exp.company}</p>
                  {exp.bullets?.length > 0 && (
                    <ul style={{ paddingLeft: '1.2em', margin: 0 }}>
                      {exp.bullets.map((b, j) => <li key={j} style={{ fontSize: 12, marginBottom: 4, color: textMuted, lineHeight: 1.65 }}>{b}</li>)}
                    </ul>
                  )}
                </div>
              ))}
            ...
            <div>
              ...
              <div style={{ marginTop: 10, display: 'flex', flexWrap: 'wrap', gap: 5 }}>
                {resume.skills.map((s, i) => (
                  <span key={i} style={{ fontSize: 10.5, padding: '3px 10px', borderRadius: 4, background: D ? '#374151' : '#f8fafc', color: textMuted, border: `1px solid ${borderCol}`, fontWeight: 500 }}>{s}</span>
                ))}
              </div>
            </div>
          )}
...
function BoldSection({ title, accent, border }: { title: string; accent: string; border: string }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
      <h2 style={{ fontSize: 13, fontWeight: 900, color: accent, margin: 0, textTransform: 'uppercase', letterSpacing: '0.05em' }}>{title}</h2>
